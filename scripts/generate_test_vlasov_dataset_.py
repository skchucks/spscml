...
fname = f"data/new_testingparams/test_parameters_mixed.csv"
...
n_train = jnp.logspace(jnp.log10(n_min.magnitude), jnp.log10(n_max.magnitude), 10)
T_train = jnp.linspace(T_min, T_max, 10)
Vp_train = jnp.linspace(Vp_min, Vp_max, 10)
...
filename = f"vlasov_testdata_results_newbounds_{date_str}.csv"
...
try:
    for i in range(len(Vp_vals)):
        ...
        T = T_vals[i]
        n = n_vals[i]
        print(f"Data point {i+1}/{len(Vp_vals)}: Vp={Vp}, T={T}, n={n}")
        try:
...
# plt.plot(n_vals, label='n values', marker='o')
# plt.yscale('log')
# plt.show()

# plt.figure(figsize=(8, 6))
# plt.plot(T_vals, label='T values', marker='o')
# plt.plot(Vp_vals, label='Vp values', marker='o')
# plt.legend()
# plt.show()
